...
const { getEmailConfig, sendTestEmail } = require('../utils/emailService');
...
router.post('/email/test-send', async (req, res) => {
  ...
  try {
    ...
    const result = await sendTestEmail(
      to,
      '[BIAW] Test email',
      'This is a test message from the Builder MVC API (Microsoft Graph).',
      '<p>This is a <strong>test</strong> message from the Builder MVC API (Microsoft Graph).</p>'
    );
    res.json({
      ok: true,
